...
class SaLoginRewards
{
    ...
    {
        $registration_achievement_id = SaRewards::$registration_achievement_id;

        SaRewards::sal_insert_reward($user->ID, $registration_achievement_id);
    ...
    function sa_badgeos_wplms_submit_course($course_id)
    {
        ...
        if ($total_courses <= 3) {
            ...
            if (!empty($achievement_id)) {
                SaRewards::sal_insert_reward($user_id, $achievement_id);
            }
        } elseif ($total_courses > 3) {
            $achievement_id_repeat = self::$course_id_4;
            SaRewards::sal_insert_reward_repeat($user_id, $achievement_id_repeat);
        } else {
            return;
        }
    }
}
class SaUnitCompleteRewards
{
        ...
        if ($curriculums <= 100) {
            switch ($curriculums) {
                case 10:
                    ...
                    SaRewards::sal_insert_reward($user_id, $unit_id_1);
                    break;
                case 25:
                    $unit_id_2 = self::$unit_id_2;
                    SaRewards::sal_insert_reward($user_id, $unit_id_2);
                    break;
                case 75:
                    $unit_id_3 = self::$unit_id_3;
                    SaRewards::sal_insert_reward($user_id, $unit_id_3);
                    break;
                case 100:
                    $unit_id_4 = self::$unit_id_4;
                    SaRewards::sal_insert_reward($user_id, $unit_id_4);
                    break;
            }
            ...
            SaRewards::sal_insert_reward_repeat($user_id, $achievement_id);
        } else {
            return;
        }
    }
